...
- **Audio Analyse Training Exports:** Training-data JSON exports keep the loaded recording baseName as `training_data_<baseName>.json`; do not append an additional random suffix because tagged ZIP baseNames are already unique and must stay easy to match.

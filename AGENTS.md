...
- **Sequence ZIP Workflow:** Tagged sheet-music sequence fixtures may live as single ZIP files containing WAV+JSON. Sidecar JSONs inside tagged ZIPs must carry stable `id` and `baseName` values derived from the ZIP base name plus timestamp fields (`recordedAt`, `updatedAt`) with date and time; run `node scripts/normalize-sequence-zip-sidecars.mjs` after adding or changing sequence ZIPs. Test helpers, sheet fingerprints, and `sfp` must keep ZIP support intact; loose `.wav/.json` pairs are compatibility inputs, not the only accepted format.
...
- **XGBoost Onset Timing Settings:** In `ml/training_config.yaml`, `labels.positive_window_ms` labels frames symmetrically around each manual onset tag, `labels.onset_tolerance_ms` scores detected onsets symmetrically around tags, and `labels.refractory_ms` controls the post-detection cooldown exported to the browser schema. Keep these concerns separate.

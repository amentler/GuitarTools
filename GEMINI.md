...
- **Do NOT edit `version.txt` manually.** The `prepare-commit-msg` hook (`scripts/auto-update-version.sh`) always regenerates and stages it automatically. Manual edits are overwritten. Also do NOT stage `version.txt` — the hook owns it. `sw.js` is also auto-synced by the same hook.

...
test.describe('Aufnahmen – Übersicht', () => {
  ...
  test('ZIP-Download aller Aufnahmen wird ausgelöst', async ({ page }) => {
    // Seed
    await page.evaluate(async () => {
      const blob = new Blob([new Uint8Array(44)], { type: 'audio/wav' });
      await new Promise((resolve, reject) => {
        ...
        req.onsuccess = e => {
          ...
          const tx = db.transaction('recordings', 'readwrite');
          tx.objectStore('recordings').put({
            baseName: 'dl_test_aaaaa',
            wavBlob: blob,
            sidecar: { chord: 'E', technique: 'strum', recordedAt: new Date().toISOString() },
          });
          tx.oncomplete = () => { db.close(); resolve(); };
          tx.onerror = () => reject(tx.error);
        };
    ...
    const [download] = await Promise.all([
      ...
      page.locator('#btn-dl-all').click(),
    ]);

    expect(download.suggestedFilename()).toMatch(/alle-aufnahmen-\d+\.zip/);
  });

  test('ZIP-Download der Auswahl wird ausgelöst', async ({ page }) => {
    // Seed
    await page.evaluate(async () => {
      const blob = new Blob([new Uint8Array(44)], { type: 'audio/wav' });
      await new Promise((resolve, reject) => {
        ...
        req.onsuccess = e => {
          ...
          const tx = db.transaction('recordings', 'readwrite');
          tx.objectStore('recordings').put({
            baseName: 'sel_dl_test_aaaaa',
            wavBlob: blob,
            sidecar: { chord: 'D', technique: 'fingerpick', recordedAt: new Date().toISOString() },
          });
          tx.oncomplete = () => { db.close(); resolve(); };
          tx.onerror = () => reject(tx.error);
        };
    ...
    const [download] = await Promise.all([
      page.waitForEvent('download'),
      page.locator('#btn-download-selected').click(),
    ]);

    expect(download.suggestedFilename()).toMatch(/aufnahmen-auswahl-\d+\.zip/);
  });
});

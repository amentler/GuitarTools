...
version.txt         – Version text shown on the main menu (format: `Version 0.X | YYYY-MM-DD HH:MM | git-hash | label`); update on each change
pages/
...
- **Update `version.txt`:** After every code/content change, you MUST update `/version.txt` with format `Version 0.X | YYYY-MM-DD HH:MM | git-hash | label` (example: `Version 0.31 | 2026-05-01 16:03 | 7f6c735 | Update`). The version number increments by 1 each commit. The git-hash is the short hash of the PREVIOUS commit (HEAD before this change). NEVER use the old `Version YYYY-MM-DD HH:MM | label` format — that was replaced in commit 19c740e.
